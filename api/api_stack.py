...
class ApiStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        CALC_MEAN_SOURCE = path.join(
            __directory, 'functions', 'calculateMean.js')
        CALC_MEDIAN_SOURCE = path.join(
            __directory, 'functions', 'calculateMedian.js')
        CALC_MODE_SOURCE = path.join(
            __directory, 'functions', 'calculateMode.js')
        MEDIAN_LAMBDA_SOURCE = path.join(
            __directory, 'calculate_median_lambda')
        QUERY_RESOLVER = path.join(
            __directory, 'resolvers', 'calculate_resolver.js')
                                 ...
                                 authorization_config=appsync.AuthorizationConfig(
                                     default_authorization=appsync.AuthorizationMode(
                                         ...
                                         api_key_config=appsync.ApiKeyConfig(
                                             ...
                                             expires=Expiration.after(
                                                 Duration.days(30)),
                                             name='Gaggle API Token')
                                     )
                                 ),
                                 log_config=appsync.LogConfig(
                                     field_log_level=appsync.FieldLogLevel.ALL, exclude_verbose_content=False)
                                 )
        ...
        query_resolver = appsync.Resolver(self, 'calculatePipeline',
                                          ...
                                          runtime=appsync.FunctionRuntime.JS_1_0_0,
                                          pipeline_config=[
                                              calculateMeanFunction, calculateMedianFunction, calculateModeFunction]
                                          )
